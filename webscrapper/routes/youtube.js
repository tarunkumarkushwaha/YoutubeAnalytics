...
const rateLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 20, // Max 10 attempts per 15 mins
...
router.post("/export", rateLimiter, verifyToken, async (req, res) => {
...
router.post("/json", rateLimiter, verifyToken, async (req, res) => {
    const videos = await fetchVideos(req.body);
    res.json({ count: videos.length, videos });
});

router.post("/keywords", rateLimiter, verifyToken, async (req, res) => {
    const videos = await fetchVideos(req.body);
